...
python scripts/log.py ~/processed_data/all_candidate_enhancers/mouse_liver_pos_ALL.narrowPeak 
```
Output: ~/processed_data/log/newRat_liver_pos_ALL.narrowPeak
...
python scripts/quantile_normalize.py \
  ~/processed_data/log/mouse_liver_pos_ALL.narrowPeak \
  ~/processed_data/log/macaque_liver_pos_ALL.narrowPeak \
  ~/processed_data/log/rat_liver_pos_ALL.narrowPeak \
  ~/processed_data/log/cow_liver_pos_ALL.narrowPeak \
  ~/processed_data/log/pig_liver_pos_ALL.narrowPeak
```
Output Directory: `~/processed_data/quantile_norm/`
...
python scripts/EQN.py mouse_liver_pos_ALL.bed ~/processed_data/
```

Output Directory: `~/processed_data/eqn/`
...
python ~/repos/atac_data_pipeline/scripts/preprocessing.py expand_peaks \
  -l 500 \
  -i ~/processed_data/log/mouse_liver_pos_ALL.narrowPeak \
  -o ~/processed_data/log/mouse_liver_pos_ALL_500bp.narrowPeak 

# 2. Extract essential columns (BED5 format: chr, start, stop, name, signal)
awk 'BEGIN{OFS="\t"} {print $1, $2, $3, $4, $7}' \
  ~/processed_data/log/mouse_liver_pos_ALL_500bp.narrowPeak > \
  ~/processed_data/log/mouse_liver_pos_ALL_500bp.bed
```
Primary output used for evaluation set creation: `~/processed_data/log/mouse_liver_pos_ALL_500bp.bed`
